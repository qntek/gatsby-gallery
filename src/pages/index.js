...
const IndexPage = ({data}) => {
	...
	const thumbnails = data.allDataJson.nodes.map((node) => {
		return <Thumbnail image={node.images.gallery.childImageSharp.gatsbyImageData} picName={node.name} artistName={node.artist.name} key={node.id} />;
	}).reverse();
  ...
  const thumbnailsToDisplay = <div className='grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4 w-full p-4 lg:p-9'>
